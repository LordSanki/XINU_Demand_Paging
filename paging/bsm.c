...
SYSCALL bsm_lookup(int pid, long vaddr, int* store, int* pageth)
{
  ...
  for(bsid=0; bsid<NBS; bsid++){
    if(bs_map[bsid].bs_status != BS_UNMAPPED){
      begin = bs_map[bsid].bs_vpno;
      end = bs_map[bsid].bs_vpno + bs_map[bsid].npages;
      if (vpno >= begin && vpno < end)
        break;
    }
  }
...
{
  if( proctab[pid].bs_map[source].bs_status != BS_UNMAPPED ){
  ...
  if(bsm_tab[source].bs_status == BS_MAPPED_PR){
    ...
    if(bsm_tab[source].bs_status == BS_UNMAPPED){
      bsm_tab[source].bs_status = BS_MAPPED_PR;
  ...
  proctab[pid].bs_map[source].bs_status = BS_MAPPED_PR;
...
SYSCALL bsm_unmap(int pid, int vpno, int flag)
{
  ...
  proctab[bsid].bs_npages = vpno - proctab[bsid].bs_vpno;;
  if(proctab[bsid].bs_npages < 1){
    proctab[bsid].bs_status = BS_UNMAPPED;
    if(bsm_tab[bsid].bs_status == BS_MAPPED_PR){
      free_bsm(bsid);
    }
    else{
      bsm_tab[bsid].bs_ref--;
